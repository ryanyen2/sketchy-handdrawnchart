define(["./hachure"], function (hachure) {
	...
	handy.prototype.line = function (x1, y1, x2, y2, delay, uuid) {
		...
		var data1 = [], data2 = [];
		data1.push({
			x: x1 + this._getOffset(-offset, offset),
			y: y1 + this._getOffset(-offset, offset)
		});
		data1.push({
			x: x1 + (x2 - x1) * divergePoint + midDispX + this._getOffset(-offset, offset),
			y: y1 + (y2 - y1) * divergePoint + midDispY + this._getOffset(-offset, offset)
		});
		data1.push({
			x: x1 + 2 * (x2 - x1) * divergePoint + midDispX + this._getOffset(-offset, offset),
			y: y1 + 2 * (y2 - y1) * divergePoint + midDispX + this._getOffset(-offset, offset)
		});
		data1.push({
			x: x2 + this._getOffset(-offset, offset),
